"""Topic classification node (brief step 6): primary topic + free-text sub-topic."""
...
def classify_topics(
    ...
    *,
    ...
    """Return ``({index: (topic, sub_topic)}, {index: error})``."""
    ...
    def call(batch: list[PostItem]) -> dict[int, tuple[str, str | None]]:
        result = invoke_prompt(
            ...
            taxonomy=taxonomies.topics,
        )
        return {r.index: (r.topic, r.sub_topic) for r in result.results}

    return run_in_batches(items, batch_size=batch_size, call=call, task="topic")
